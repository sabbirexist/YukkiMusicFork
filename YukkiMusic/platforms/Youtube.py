...
def _ytdlp_search_sync(query: str, limit: int = 1):
    """Search YouTube via yt-dlp and return results shaped like the old
    youtubesearchpython VideosSearch().next()["result"] list."""
    ydl_opts = {
        "quiet": True,
        "no_warnings": True,
        ...
        results.append(
            {
                ...
                "viewCount": {"short": _format_views(entry.get("view_count"))},
                "channel": {
                    "name": entry.get("channel") or entry.get("uploader") or "Unknown Channel",
                    "link": entry.get("channel_url") or entry.get("uploader_url") or "",
                },
...
async def yt_search(query: str, limit: int = 1):
    """Async wrapper around the yt-dlp search helper."""
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(None, _ytdlp_search_sync, query, limit)
...
class YouTubeAPI:
    ...
    async def details(
        ...
        if "&" in link:
            link = link.split("&")[0]
        results = await yt_search(link, limit=1)
    ...
    async def slider(
        ...
        if "&" in link:
            link = link.split("&")[0]
        result = await yt_search(link, limit=10)
